...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'bc_count_contigs'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from bc_count_contigs.bc_count_contigsImpl import bc_count_contigs
impl_bc_count_contigs = bc_count_contigs(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['bc_count_contigs.count_contigs_async'] = ['bc_count_contigs', 'count_contigs']
async_check_methods['bc_count_contigs.count_contigs_check'] = ['bc_count_contigs', 'count_contigs']
sync_methods['bc_count_contigs.count_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'bc_count_contigs'
        ...
        self.rpc_service.add(impl_bc_count_contigs.count_contigs,
                             name='bc_count_contigs.count_contigs',
                             types=[basestring, basestring])
        self.method_authentication['bc_count_contigs.count_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "bc_count_contigs but no authentication header was passed"
                            raise err
